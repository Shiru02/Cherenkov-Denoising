...
class PaletteLoss(nn.Module):
    ...
    def forward(self, eps_pred, eps_target) -> tuple:
        ...
        # 2. Frequency Loss (Spectral Amplitude Matching)
        # Compute Real 2D Fast Fourier Transform. 
        # norm='ortho' scales the transform to preserve energy.
        fft_pred = torch.fft.rfft2(eps_pred, norm='ortho')
        fft_target = torch.fft.rfft2(eps_target, norm='ortho')
        
        # Extract amplitude/magnitude (ignoring phase for stability)
        mag_pred = torch.abs(fft_pred)
        mag_target = torch.abs(fft_target)
        
        # L1 distance between frequency amplitudes
        loss_freq = F.l1_loss(mag_pred, mag_target)
