...
=================================================================
x0-Parameterization + Temporal Attention Modulation (TAM) + Combined Loss:
...
- Combined Loss: L1 + SSIM + Spectral Amplitude (Frequency) Loss.
  - L1: Pixel-level accuracy without the mean-regression bias of L2.
  - SSIM: Preserves local structural contrast and luminance patterns.
  - FreqLoss: Penalizes spectral magnitude mismatch via log-compressed 2D FFT.
...
def ssim_loss(x: torch.Tensor, y: torch.Tensor, 
              window_size: int = 7, C1: float = 0.01**2, C2: float = 0.03**2) -> torch.Tensor:
    """
    Differentiable SSIM loss: returns 1 - SSIM (so lower is better).
    Uses a uniform averaging window for simplicity and stability.
    Operates on (B, C, H, W) tensors with values in [0, 1].
    """
    pad = window_size // 2
...
class PaletteLoss(nn.Module):
    """
    Combined x0-Parameterization Loss for anti-blur denoising:
      L_total = l1_weight * L1 + ssim_weight * (1 - SSIM) + freq_weight * L_freq

    - L1: Pixel accuracy without L2's mean-regression bias.
    - SSIM: Preserves local structural contrast and luminance.
    - L_freq: Log-spectral amplitude matching via 2D rFFT.
    """
    def __init__(self, l1_weight=1.0, ssim_weight=0.1, freq_weight=0.05):
        super().__init__()
    ...
    def forward(self, x0_pred, x0_target) -> tuple:
        # 1. L1 Loss - Primary pixel-level objective
        l1 = F.l1_loss(x0_pred, x0_target)
        
        # 2. SSIM Loss - Structural similarity (1 - SSIM, so lower = better)
        loss_ssim = ssim_loss(x0_pred.clamp(0, 1), x0_target)
        
        # 3. Frequency Loss - Log-Spectral Amplitude Matching
        # Using rfft2 for efficiency on real-valued image data
        fft_pred   = torch.fft.rfft2(x0_pred, norm='ortho')
        fft_target = torch.fft.rfft2(x0_target, norm='ortho')
        
        # log1p compresses dynamic range so high-frequency detail
        # isn't drowned out by dominant low-frequency energy
        mag_pred   = torch.log1p(torch.abs(fft_pred))
        mag_target = torch.log1p(torch.abs(fft_target))
        loss_freq  = F.mse_loss(mag_pred, mag_target)
...
class PaletteUNet(nn.Module):
    def __init__(self, base_channels: int = 64, embed_dim: int = 128):
        ...
        # --- Temporal Attention Modulation (TAM) Components ---
        self.ch_b = ch * 8
    ...
    def forward(self, x_t, y, t, noise_level, history=None):
        ...
        # Return BOTH the prediction and the pooled bottleneck state
...
class TrainingConfig:
    def __init__(self):
        ...
        self.save_dir  = "experiments/palette_cherenkov_tam_freq"
        ...
        # --- Loss Weights ---
        self.l1_weight   = 1.0   # Primary pixel-level loss
        self.ssim_weight = 0.1   # Structural similarity (anti-blur)
        self.freq_weight = 0.05  # Spectral amplitude matching
        # --------------------
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        for batch in pbar:
            ...
            epoch_l1.append(metrics['l1'])
            epoch_ssim.append(metrics['ssim'])
            epoch_freq.append(metrics['loss_freq'])
            
            self.writer.add_scalar('Train/Loss_Total', metrics['total'], self.global_step)
            self.writer.add_scalar('Train/Loss_L1', metrics['l1'], self.global_step)
            self.writer.add_scalar('Train/Loss_SSIM', metrics['ssim'], self.global_step)
    ...
    def evaluate(self, epoch):
        ...
        for batch in self.val_loader:
            ...
            val_l1.append(metrics['l1'])
            val_ssim.append(metrics['ssim'])
            val_freq.append(metrics['loss_freq'])
        ...
        mean_l1   = float(np.mean(val_l1))
        mean_ssim = float(np.mean(val_ssim))
        mean_freq = float(np.mean(val_freq))
        mean_psnr = float(np.mean(psnrs))
        
        self.writer.add_scalar('Val/Loss_Total', mean_loss, epoch)
        self.writer.add_scalar('Val/Loss_L1', mean_l1, epoch)
        self.writer.add_scalar('Val/Loss_SSIM', mean_ssim, epoch)
        self.writer.add_scalar('Val/Loss_Freq', mean_freq, epoch)
        self.writer.add_scalar('Val/PSNR', mean_psnr, epoch)
        
        return mean_loss, mean_psnr, mean_l1, mean_ssim, mean_freq

    def log_epoch(self, epoch, tr_loss, val_loss, val_psnr, val_l1, val_ssim, val_freq):
        lr = self.optimizer.param_groups[0]['lr']
        self.logger.info(
            f"Epoch {epoch} | Tr Loss: {tr_loss:.4f} | "
            f"Val Loss: {val_loss:.4f} | Val PSNR: {val_psnr:.2f} | "
            f"L1: {val_l1:.4f} | SSIM: {val_ssim:.4f} | Freq: {val_freq:.4f} | LR: {lr:.2e}"
        )
        df = pd.DataFrame([{
            'epoch': epoch, 'train_loss': tr_loss, 
            'val_loss': val_loss, 'val_psnr': val_psnr, 'lr': lr,
            'val_l1': val_l1, 'val_ssim': val_ssim, 'val_freq': val_freq,
        }])
    ...
    def plot_curves(self):
        ...
        ax1.set_title('Total Loss & PSNR')

        # --- Right Plot: Component Losses ---
        if 'val_l1' in df.columns:
            ax3.set_xlabel('Epoch')
            ax3.set_ylabel('Component Loss')
            ax3.plot(df['epoch'], df['val_l1'], label='Val L1', color='tab:blue', linestyle='-')
            ax3.plot(df['epoch'], df['val_ssim'], label='Val SSIM', color='tab:purple', linestyle='-')
            ax3.plot(df['epoch'], df['val_freq'], label='Val Freq', color='tab:red', linestyle='-')
    ...
    def run(self, start_epoch=1):
        for epoch in range(start_epoch, self.config.num_epochs + 1):
            ...
            if epoch % self.config.eval_freq == 0:
                val_loss, val_psnr, val_l1, val_ssim, val_freq = self.evaluate(epoch)
                self.log_epoch(epoch, train_loss, val_loss, val_psnr, val_l1, val_ssim, val_freq)
                self.plot_curves()
...
def main():
    ...
    config.save_dir  = "experiments/palette_cherenkov_tam_freq"
    # ============================================================
    ...
    # Note: If resuming from the previous TAM checkpoint, strict=False inside load_checkpoint
    # ensures that new layers are safely initialized while preserving your previous weights!
