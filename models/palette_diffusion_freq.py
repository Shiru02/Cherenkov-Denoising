...
class PaletteLoss(nn.Module):
    """
    Stabilized Combined L2 noise-matching and Log-Spectral Amplitude Loss.
    """
    def __init__(self, freq_weight: float = 0.01):
        super().__init__()
        self.freq_weight = freq_weight

    def forward(self, eps_pred, eps_target) -> tuple:
        # 1. Spatial Loss (MSE) - Primary objective for PSNR
        mse_spatial = F.mse_loss(eps_pred, eps_target)
        
        # 2. Frequency Loss (Log-Spectral Matching)
        # Using rfft2 for efficiency on real-valued image data
        fft_pred = torch.fft.rfft2(eps_pred, norm='ortho')
        fft_target = torch.fft.rfft2(eps_target, norm='ortho')
        
        # Apply log1p (log(1+x)) to the magnitude to compress the dynamic range.
        # This prevents the DC component and low frequencies from dominating 
        # the high-frequency "details" we want to preserve.
        mag_pred = torch.log1p(torch.abs(fft_pred))
        mag_target = torch.log1p(torch.abs(fft_target))
        
        # Use MSE in frequency domain for smoother gradients compared to L1
        loss_freq = F.mse_loss(mag_pred, mag_target)
        
        # Total combined loss
        total_loss = mse_spatial + (self.freq_weight * loss_freq)
...
class TrainingConfig:
    def __init__(self):
        ...
        self.save_dir  = "experiments/palette_cherenkov_mc_freq"
        ...
        self.freq_weight = 0.1 # Weight for Spectral Amplitude Loss
...
class Trainer:
    ...
    def _setup_model(self):
        ...
        self.criterion = PaletteLoss(freq_weight=self.config.freq_weight) 
...
def main():
    ...
    config.save_dir  = "experiments/palette_cherenkov_mc_freq"
    # ============================================================
