...
- Resumes training safely bypassing PyTorch 2.6 weights_only NumPy restrictions.
"""
...
class DiffusionSampler:
    ...
    def ddpm_sample(self, y, noise_level):
        ...
        for t_val in tqdm(reversed(range(T)), desc="DDPM (x0-pred)", total=T, leave=False):
            t_tensor = torch.full((B,), t_val, device=self.device, dtype=torch.long)
            
            # 1. Predict x0
            x0_pred = self.model(x_t, y, t_tensor, noise_level)
            ...
            # 2. Derive implied epsilon for standard DDPM equation
    ...
    def ddim_sample(self, y, noise_level, n_steps=100, eta=0.0):
        ...
        for i, t_val in enumerate(tqdm(timesteps, desc=f"DDIM η={eta} (x0-pred)", leave=False)):
            t_tensor = torch.full((B,), t_val, device=self.device, dtype=torch.long)
...
class TrainingConfig:
    def __init__(self):
        ...
        self.save_dir  = "experiments/palette_cherenkov_refactored"
...
class Trainer:
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        # FIX: weights_only=False bypasses PyTorch 2.6's block on the numpy float!
    ...
    def train_epoch(self, epoch):
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast('cuda', enabled=self.config.mixed_precision):
                # FIX: Predict x0 directly and train against x0
                x0_pred = self.model(x_t, y, t, nl)
    ...
    def evaluate(self, epoch):
        ...
        for batch in self.val_loader:
            ...
            with torch.amp.autocast('cuda', enabled=self.config.mixed_precision):
                # FIX: Predict x0 directly
                x0_pred = self.model(x_t, y, t, nl)
                loss, metrics = self.criterion(x0_pred, x0)

            val_losses.append(metrics['total'])
            
            # Since x0 is directly predicted, clamping is safe and simple
            mse_val = F.mse_loss(x0_pred.clamp(0, 1), x0).item()
            psnrs.append(10 * math.log10(1.0 / (mse_val + 1e-8)))

        # Convert back to standard python float to avoid numpy object pickling issues
