...
# ==============================================================================
try:
    from palette_diffusion_freq import PaletteUNet, DiffusionSchedule, DiffusionSampler
except ImportError:
    print("Error: Could not import from palette_diffusion_freq.py")
...
def save_per_image_profile(records: list, save_path: str):
    """
    Line plot showing all key metrics per image.
    """
...
def evaluate_dataset(sampler: DiffusionSampler, args, device: str = 'cuda'):
    ...
    if len(mae_tex)  > 0: print(f"Average MAE (Texture):     {mae_tex.mean():.6f}")
    if len(mae_flat) > 0: print(f"Average MAE (Flat):        {mae_flat.mean():.6f}")
...
def parse_args():
    p = argparse.ArgumentParser(description='Evaluation script for Palette Diffusion (Freq) on Full Images')

    p.add_argument('--checkpoint', type=str,
                   default=r"E:\Chr_denoise\experiments\palette_cherenkov_mc_freq\checkpoints\best_model.pth",
    ...
    p.add_argument('--output_dir', type=str,
                   default=r"E:\Chr_denoise\palette_freq_evaluation_results_full",
...
def main():
    ...
    model = PaletteUNet(base_channels=64, embed_dim=128, dropout=0.1).to(device)
