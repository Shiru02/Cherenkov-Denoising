"""
Evaluation script for Palette-Style Cherenkov Diffusion
======================================================
...
Saves ALL denoised images and comparison figures.

FIX: Properly handles 16-bit TIFF images (uint16, 0-65535 range).
     The original code used .convert('L') which forces 16-bit data
     into 8-bit, destroying the dynamic range and causing artifacts.
...
try:
    ...
    print("Error: Could not import from palette_diffusion_denoise_noise.py")
...
def load_image_as_float(path: str) -> np.ndarray:
    ...
    Handles:
      - 8-bit  (uint8,  0–255)   → divide by 255
      - 16-bit (uint16, 0–65535) → divide by 65535
      - Other dtypes              → divide by observed max (fallback)
    
    Does NOT call .convert('L'), which would silently collapse 16-bit to 8-bit.
    If the image has multiple channels, it is converted to single-channel via
    a luminance formula after reading the raw data.
    """
    img = Image.open(path)
    arr = np.array(img)

    # Handle multi-channel images (e.g., RGB TIFF) by converting to grayscale
    if arr.ndim == 3:
        # Standard luminance weights
        arr = (0.2989 * arr[:, :, 0] + 0.5870 * arr[:, :, 1] + 0.1140 * arr[:, :, 2])

    # Determine the correct normalisation factor from dtype
    if arr.dtype == np.uint8:
        ...
        # Already float – assume [0,1]; clip just in case
        return np.clip(arr.astype(np.float32), 0.0, 1.0)
    else:
        # Fallback: normalise by observed maximum
        obs_max = arr.max()
...
def save_image_matching_depth(arr_float: np.ndarray, save_path: str, reference_path: str):
    ...
    reference image.  For 16-bit references the output is uint16; otherwise uint8.
    """
...
def save_evaluation_sample(noisy: np.ndarray, denoised: np.ndarray, gt: np.ndarray, save_path: str):
    ...
    Save a 2-row comparison figure:
      Row 1: Noisy | Denoised | Ground Truth
      Row 2: |Noisy − GT| | |Denoised − GT| | Colorbar reference
    
    Residuals show absolute error using the 'inferno' colormap (dark=low error,
    bright=high error). Both residual maps share the same color scale so they
    are directly comparable.
    """
    abs_residual_noisy    = np.abs(noisy - gt)
    abs_residual_denoised = np.abs(denoised - gt)

    # Shared color range so both residuals are on the same scale
    vmax = max(np.max(abs_residual_noisy), np.max(abs_residual_denoised))
    if vmax < 1e-8:
        vmax = 1.0  # avoid degenerate case
    ...
    im0 = axes[1, 0].imshow(abs_residual_noisy, cmap='inferno', vmin=0, vmax=vmax)
    noisy_mae = np.mean(abs_residual_noisy)
    axes[1, 0].set_title(f"|Noisy − GT|  (MAE={noisy_mae:.4f})")
    axes[1, 0].axis("off")

    im1 = axes[1, 1].imshow(abs_residual_denoised, cmap='inferno', vmin=0, vmax=vmax)
    denoised_mae = np.mean(abs_residual_denoised)
    axes[1, 1].set_title(f"|Denoised − GT|  (MAE={denoised_mae:.4f})")
    axes[1, 1].axis("off")

    # Colorbar in the third column of row 2
    axes[1, 2].axis("off")
    cbar = fig.colorbar(im1, ax=axes[1, 2], fraction=0.6, pad=0.05, shrink=0.8)
    cbar.set_label("Absolute pixel error (dark = low, bright = high)", fontsize=10)
...
def save_per_image_profile(records: list, save_path: str):
    """
    Bar plots showing all key metrics per image — reveals which images are
    ...
    axes[0].set_title('Per-Image Metric Profile (Palette-Diffusion Noise)', fontsize=13)
...
def save_texture_vs_flat_plot(records: list, save_path: str):
    ...
    ax.set_title('MAE: Texture-Rich vs Flat Regions per Image (Palette-Diffusion Noise)', fontsize=13)
...
def save_frequency_analysis_plot(records: list, save_path: str):
    ...
    ax.set_title('Frequency Error: Low (Structure) vs High (Details) per Image (Palette-Diffusion Noise)', fontsize=13)
    ax.legend(fontsize=10)
    ax.grid(axis='y', alpha=0.3)
...
def save_residual_analysis_plot(records: list, save_path: str):
    ...
    plt.suptitle('Noise Residual Analysis (pred − GT) — Palette-Diffusion Noise', fontsize=13, y=1.01)
    plt.tight_layout()
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()
...
def _build_blending_window(size: int, mode: str = 'gaussian', sigma_fraction: float = 0.25) -> np.ndarray:
    """
    Build a 2D soft blending window of shape (size, size).
    
    Modes:
      'gaussian'  – 2D Gaussian bell; sigma = size * sigma_fraction
      'cosine'    – raised-cosine (Hann) window; no extra parameter needed
    
    The window is always normalised so its peak = 1.0.
    """
    if mode == 'gaussian':
    ...
    # Add small epsilon to avoid division by zero at corners
...
def process_whole_image(noisy_np: np.ndarray, sampler: DiffusionSampler, args, device: str) -> np.ndarray:
    ...
    Instead of uniform averaging (which creates abrupt weight transitions at
    patch boundaries), each denoised patch is multiplied by a 2D Gaussian
    window before accumulation.  The window peaks at 1.0 in the center and
    tapers smoothly toward the edges, so overlap zones blend gradually with
    no visible seams.
    """
    h, w = noisy_np.shape
    ...
    # Pre-compute the blending window (same for every patch)
...
def evaluate_dataset(sampler: DiffusionSampler, args, device: str = 'cuda'):
    ...
    print(f"Starting Diffusion DDIM evaluation on {len(noisy_files)} FULL images...")
    ...
    for i, noisy_path in enumerate(noisy_files):
        ...
        # Load images with proper bit-depth handling (no .convert('L'))
        ...
        # RUN THE WHOLE-IMAGE PIPELINE
        denoised_np = process_whole_image(noisy_np, sampler, args, device)

        # Save denoised image matching the original bit-depth
        ...
        # Accumulate regional data for aggregate analysis
    ...
    # Regional analysis with bootstrap CIs
    lo, mean, hi = bootstrap_ci(all_regional_mae)
    ...
    # Bootstrap CIs for whole-image metrics
    ...
    # 1. Bootstrap bar charts for key metrics
    ...
    print(f"         texture_vs_flat.png, frequency_analysis.png, residual_analysis.png")
...
def parse_args():
    p = argparse.ArgumentParser(description='Evaluation script for Palette Diffusion on Full Images')
    
    p.add_argument('--checkpoint', type=str, 
                   default=r"E:\Chr_denoise\experiments\palette_cherenkov_refactored\checkpoints\best_model.pth",
    ...
    p.add_argument('--output_dir', type=str, 
                   default=r"E:\Chr_denoise\palette_evaluation_results_full",
    ...
    p.add_argument('--stride', type=int, default=64,
                   help='Stride for patch extraction. If less than patch_size, patches will overlap and be averaged to prevent seams (e.g., 128 or 200).')
...
def main():
    ...
    if 'model' in ckpt:
        ...
        print(f" Loaded model checkpoint from epoch {ckpt.get('epoch', '?')}")
    else:
