"""
Evaluation script for Palette-Style Cherenkov Diffusion (TAM + SSIM + Freq)
===========================================================================
...
Features:
  ...
  - TAM temporal history buffer support
...
try:
    ...
    print("Error: Could not import from palette_TAM_SSIM_freq.py")
...
def save_evaluation_sample(noisy: np.ndarray, denoised: np.ndarray, gt: np.ndarray, save_path: str):
    ...
    # Shared color range so both absolute residuals are on the same scale
    vmax_abs = max(np.max(abs_residual_noisy), np.max(abs_residual_denoised))
    if vmax_abs < 1e-8:
        vmax_abs = 1.0
        
    # Diverging color range for raw residual (symmetric around 0)
...
def save_summary_radar(record_means: dict, save_path: str):
    ...
    for name, higher_better in metric_info:
        if name in record_means:
            val = record_means[name]
            # Normalise: for "lower is better" metrics, invert
            # We just show the raw value — the radar shows the profile shape
            labels.append(name)
            values.append(val)
    ...
    for i, (name, higher_better) in enumerate(metric_info):
        if name not in record_means:
            continue
        v = values[len(norm_vals)]
        # Simple min-max with small buffer
        norm_vals.append(v)
    
    # For radar, normalise to [0,1] range
...
def evaluate_dataset(sampler: DiffusionSampler, args, device: str = 'cuda'):
    ...
    print(f"Starting TAM+SSIM+Freq Diffusion DDIM evaluation on {len(noisy_files)} FULL images...")
    ...
    for i, noisy_path in enumerate(noisy_files):
        ...
        # Load images with proper bit-depth handling
        ...
        # Save comparison figure with residual maps
...
def parse_args():
    p = argparse.ArgumentParser(description='Evaluation script for Palette Diffusion (TAM + SSIM + Freq) on Full Images')
    
    p.add_argument('--checkpoint', type=str, 
                   default=r"E:\Chr_denoise\experiments\palette_cherenkov_tam_freq\checkpoints\best_model.pth",
    ...
    p.add_argument('--output_dir', type=str, 
                   default=r"E:\Chr_denoise\palette_tam_freq_evaluation_results_full",
    ...
    # TAM-specific settings
    p.add_argument('--tam_history_size', type=int, default=5,
                   help='Size of the TAM temporal memory buffer (default: 5, matching training config)')
...
def main():
    ...
    # Initialize architecture from your TAM+SSIM+Freq training file
    ...
    if 'model' in ckpt:
        ...
        print(f" Loaded model checkpoint from epoch {ckpt.get('epoch', '?')} (Best Val Loss: {ckpt.get('best_loss', 'N/A')})")
    else:
        model.load_state_dict(ckpt) 
        
    model.eval()

    # Initialize Schedule & Sampler with TAM history size
    schedule = DiffusionSchedule(T=1000).to(device)
    sampler = DiffusionSampler(schedule, model, device, tam_history_size=args.tam_history_size)
