"""
Evaluation script for UNetMCDropout Cherenkov Denoiser — Full Images
====================================================================
Evaluates whole images of arbitrary sizes by automatically
cropping them into patches, denoising via batched MC inference,
...
Features:
  ...
  - MC Dropout uncertainty estimation (n_samples forward passes)
...
def save_evaluation_sample(noisy: np.ndarray, denoised: np.ndarray, gt: np.ndarray, save_path: str):
    ...
    axes[0, 1].set_title("Denoised (UNet MC Dropout)")
    axes[0, 1].axis("off")
...
def save_bootstrap_bar_chart(values: list, metric_name: str, save_path: str,
    ...
    ax.set_xticks([0])
    ax.set_xticklabels(['UNet MC Dropout'], fontsize=11)
...
def save_per_image_profile(records: list, save_path: str):
    ...
    axes[0].set_title('Per-Image Metric Profile (UNet MC Dropout)', fontsize=13)
...
def save_texture_vs_flat_plot(records: list, save_path: str):
    ...
    ax.set_title('MAE: Texture-Rich vs Flat Regions per Image (UNet MC Dropout)', fontsize=13)
...
def save_frequency_analysis_plot(records: list, save_path: str):
    ...
    ax.set_title('Frequency Error: Low (Structure) vs High (Details) per Image (UNet MC Dropout)', fontsize=13)
    ax.legend(fontsize=10)
    ax.grid(axis='y', alpha=0.3)
...
def save_residual_analysis_plot(records: list, save_path: str):
    ...
    plt.suptitle('Noise Residual Analysis (pred − GT) — UNet MC Dropout', fontsize=13, y=1.01)
    plt.tight_layout()
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()
...
def process_whole_image(noisy_np: np.ndarray, model: UNetMCDropout, args, device: str) -> np.ndarray:
    """
    Crops a full image into overlapping patches, runs UNet MC Dropout denoising
    on batches, and stitches them back using soft (Gaussian) windowed blending.

    For each batch of patches, runs n_samples MC forward passes and averages
    the mean predictions — identical to mc_uncertainty but batched.
    """
    h, w = noisy_np.shape
    ...
    print(f"    -> MC samples per patch: {n_samples} | Blending mode: {blend_mode}")
    ...
    for i in range(0, len(patches), args.batch_size):
        ...
        # MC Dropout: run n_samples forward passes and average
        mc_accum = torch.zeros_like(batch_tensor)
        with torch.no_grad():
            for _ in range(n_samples):
                mean_pred, _ = model(batch_tensor, batch_nl)
                mc_accum += mean_pred
        mc_mean = mc_accum / n_samples

        denoised_patches.extend(mc_mean.squeeze(1).cpu().numpy())
...
def evaluate_dataset(model: UNetMCDropout, args, device: str = 'cuda'):
    ...
    print(f"Starting UNet MC Dropout evaluation on {len(noisy_files)} FULL images...")
    ...
    for i, noisy_path in enumerate(noisy_files):
        ...
        denoised_np = process_whole_image(noisy_np, model, args, device)
...
def parse_args():
    p = argparse.ArgumentParser(description='Evaluation script for UNet MC Dropout on Full Images')

    p.add_argument('--checkpoint', type=str,
                   default=r"E:\Chr_denoise\experiments\unet_mc_dropout\checkpoints\best_model.pt",
    ...
    p.add_argument('--output_dir', type=str,
                   default=r"E:\Chr_denoise\unet_mc_evaluation_results_full",
                   help='Where to save results (metrics CSV and image samples)')

    p.add_argument('--patch_size', type=int, default=128,
                   help='Size of the patches to extract (should match your training size)')
    ...
    p.add_argument('--n_samples', type=int, default=20,
                   help='Number of MC forward passes per patch (default: 20)')
    ...
    p.add_argument('--blend_mode', type=str, default='gaussian', choices=['gaussian', 'cosine'],
                   help='Blending window for patch stitching.')
...
def main():
    ...
    # Load model using the checkpoint's stored config
    ...
    print(f" Loaded UNet MC Dropout checkpoint  |  epoch {epoch}  |  val PSNR {val_psnr:.2f} dB")

    evaluate_dataset(model, args, device=device)
