...
import cv2
import network_emb
import network_emb_uncertainty
import network_emb_uncertainty_FT
import network
import scipy.io
...
def create_generator(opt):
    if opt.pre_train:
        ...
        # PyTorch 2.6 changed torch.load default to weights_only=True; older checkpoints
        # may have been saved as full nn.Module objects. Only set weights_only=False for trusted files.
        pretrained_net = torch.load(opt.load_name + '.pth', weights_only=False, map_location='cpu')
...
def create_MyDNN(opt):
    # Initialize the network
    MyDNN = network_emb_uncertainty.MyDNN(opt)
    # Init the network
    network_emb_uncertainty.weights_init(MyDNN, init_type = opt.init_type, init_gain = opt.init_gain)
    print('MyDNN is created!')
    return MyDNN

def create_MyDNN_FT(opt):
    # Initialize the network
    MyDNN_FT = network_emb_uncertainty_FT.MyDNN(opt)
    # Init the network
    network_emb_uncertainty_FT.weights_init(MyDNN_FT, init_type = opt.init_type, init_gain = opt.init_gain)
    print('MyDNN_FT is created!')
    return MyDNN_FT
